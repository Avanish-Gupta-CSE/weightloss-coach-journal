...
export default function NutritionPage() {
  ...
  const macroData = [
    { name: 'Protein', value: avgProtein * 4, color: '#22c55e' },
    { name: 'Carbs', value: 170 * 4, color: '#3b82f6' },
    { name: 'Fat', value: 56 * 9, color: '#eab308' },
  ];
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className="bg-bg-secondary rounded-lg border border-bg-tertiary/50 p-4">
          ...
          <div className="text-2xl font-bold text-accent-purple">
            {periodData.filter(d => d.protein > 0).length}/{periodData.length}
          </div>
        </div>
      </div>
          ...
          {problemDays.slice(0, 10).map(day => (
            <div key={day.day} className="flex items-center justify-between p-3 bg-bg-tertiary/20 rounded-lg">
              ...
              <div className="flex items-center gap-3">
                <span className={`font-bold ${getProteinColor(day.protein || 0)}`}>{day.protein}g</span>
