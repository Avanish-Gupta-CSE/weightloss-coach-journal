...
export default function ProteinChart() {
  ...
  const target = macroTargets?.protein || 180;
  ...
  const CustomTooltip = ({ active, payload }) => {
    ...
    return (
      <div className="bg-bg-secondary border border-bg-tertiary rounded-lg p-3 shadow-lg">
        ...
        <p className={`text-xs mt-1 ${item.protein >= 150 ? 'text-accent-green' : item.protein >= 120 ? 'text-accent-yellow' : 'text-accent-red'}`}>
          {item.protein >= 150 ? 'Good' : item.protein >= 120 ? 'Warning' : 'Low'}
        </p>
      </div>
    );
  };
    ...
    <Card title="Protein Intake (Last 14 Days)" icon={Beef} className="mb-6">
      <div className="flex items-center justify-between mb-4">
        <div className="text-sm text-text-secondary">
          7-day avg: <span className={`font-bold ${avgProtein >= 150 ? 'text-accent-green' : avgProtein >= 120 ? 'text-accent-yellow' : 'text-accent-red'}`}>{avgProtein}g</span>
        </div>
