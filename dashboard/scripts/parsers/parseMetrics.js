...
export function parseMetrics(filePath) {
  ...
  const weighins = weighinRows.map(row => {
    ...
    if (weightStr.includes('SKIPPED') || weightStr.includes('MISSED')) {
      type = 'skipped';
    } else if (weightStr.includes('spot') || weightStr.includes('unofficial')) {
      type = 'spot-check';
    } else {
